...
The bigger problem is UI inconsistencies. Imagine product asks for all download buttons to use an updated `download.svg`. If that file exists in two separate spots in the codebase, whichever team picks up the ticket won't realize they need to replace both copies — leaving part of the UI with stale styling. Over time, duplicates also accumulate into sizable chunks of wasted space on the server or CDN.
